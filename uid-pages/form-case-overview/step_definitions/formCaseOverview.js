...
const archivedCaseUrl = 'API/bpm/archivedCase';
const openCaseUrl = 'API/bpm/case';
const archivedCasePage = urlPrefix + 'resources/index.html?id=30003';
const openCasePage = urlPrefix + 'resources/index.html?id=30004';
const caseUrlWithoutId = urlPrefix + 'resources/index.html?id=';
...
given('The archived case {string} server response is defined', (archivedCaseId) => {
    ...
    cy.route({
        method: 'GET',
        url: urlPrefix + archivedCaseUrl + '?c=1&d=started_by?d=startedBySubstitute&d=processDefinitionId&f=sourceObjectId=' + archivedCaseId + '&p=0',
...
given('The archived case {string} empty context server response is defined', (archivedCaseId) => {
    ...
    cy.route({
        method: 'GET',
        url: urlPrefix + archivedCaseUrl + '/' + archivedCaseId + '/context',
...
given('The archived case {string} empty document server response is defined', (archivedCaseId) => {
    ...
    cy.route({
        method: 'GET',
        url: urlPrefix + 'API/bpm/archivedCaseDocument?f=caseId=' + archivedCaseId,
...
given('The open case {string} server response is defined', (openCaseId) => {
    ...
    cy.route({
        method: 'GET',
        url: urlPrefix + openCaseUrl + '/' + openCaseId + '?d=started_by?d=startedBySubstitute&d=processDefinitionId',
        response: '@openCase'
    }).as('openCaseRoute');
});
...
given('The open case {string} empty context server response is defined', (openCaseId) => {
    ...
    cy.route({
        method: 'GET',
        url: urlPrefix + openCaseUrl + '/' + openCaseId + '/context',
...
given('The open case {string} empty document server response is defined', (openCaseId) => {
    ...
    cy.route({
        method: 'GET',
        url: urlPrefix + 'API/bpm/caseDocument?f=caseId=' + openCaseId,
...
given('A list of executed tasks server response is defined', () => {
    ...
    cy.route({
        method: 'GET',
        url: urlPrefix + 'API/bpm/task?p=0&c=999&d=executedBy&d=executedBySubstitute*',
        response: '@openTask'
    }).as('openTaskRoute');
    cy.fixture('json/archivedTaskExecutedByUser.json').as('archivedTaskExecutedByUser');
    cy.route({
        method: 'GET',
        url: urlPrefix + 'API/bpm/archivedHumanTask?p=0&c=999&d=executedBy&d=executedBySubstitute*',
...
when('I visit the archived case index page', () => {
    cy.visit(archivedCasePage);
});

when('I visit the open case index page', () => {
    cy.visit(openCasePage);
});
...
then('I can see both IDs have correct values', () => {
    cy.wait('@archivedCaseRoute').then(() => {
